# https://github.com/yt-dlp/yt-dlp/pull/11485
...
from yt_dlp.postprocessor.embedthumbnail import EmbedThumbnailPP, EmbedThumbnailPPError
...
def run_patched(self, info):
    ...
    if info["ext"] == "mp3":
        ...
        options.extend(
            [
                "-attach",
                self._ffmpeg_filename_argument(thumbnail_filename),
                f"-metadata:s:{new_stream}",
                f"mimetype={mimetype}",
                f"-metadata:s:{new_stream}",
                f"filename=cover.{thumbnail_ext}",
            ]
        )
        self._report_run("ffmpeg", filename)
        ...
        if not success:
            ...
            atomicparsley = next(
                (
                    ...
                    x
                    for x in ["AtomicParsley", "atomicparsley", "libatomicparsley.so"]
                    if check_executable(x, ["-v"])
                ),
                None,
            )
        ...
        if not mutagen:
            raise EmbedThumbnailPPError(
                "module mutagen was not found. Please install using `python3 -m pip install mutagen`"
            )
        self._report_run("mutagen", filename)
        ...
        raise EmbedThumbnailPPError(
            "Supported filetypes for thumbnail embedding are: mp3, mkv/mka, ogg/opus/flac, m4a/mp4/m4v/mov, aiff/wav"
        )
...
EmbedThumbnailPP.run = run_patched
